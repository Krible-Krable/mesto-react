export default function PopupWithForm(props) {
    return (
        <>
            <section className={`popup popup_profile popup_${props.name}`}>
                <div className="popup__container">
                    <button className="popup__button-close" type="button"></button>
                    <form className="popup__form" name={props.name} noValidate>
                        <h2 className="popup__title">Редактировать профиль{props.title}</h2>
            ...
            <section className={`popup popup_card popup_${props.name}`}>
                <div className="popup__container">
                    <button className="popup__button-close" type="button"></button>
                    <form className="popup__form" name={props.name} noValidate>
                        <h2 className="popup__title">Новое место{props.title}</h2>
            ...
            <section className={`popup popup_get-delete popup_${props.name}`}>
                <div className="popup__container">
                    <button className="popup__button-close" type="button"></button>
                    <form className="popup__form popup__form_type_delete" name={props.name}>
                        <h2 className="popup__title">Вы уверены?{props.title}</h2>
                        <button type="submit" value="Да" className="popup__button popup__button-delete">Да</button>
                    </form>
                </div>
            </section>

            <section className={`popup popup_edit-avatar popup_${props.name}`}>
                <div className="popup__container popup__avatar">
                    <button className="popup__button-close" type="button"></button>
                    <form className="popup__form" name={props.name}>
                        <h2 className="popup__title">Обновить аватар{props.title}</h2>
